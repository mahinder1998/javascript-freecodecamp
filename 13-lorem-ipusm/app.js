const text = [
  ...
  form.addEventListener("submit", function(e){
    ...
    // Note: yahn pe check karange
    // 1st input ki value empty 
    // 2nd input ki value less the 1 (eg. -1)
    // 3rs input ki value > 9
    // es case me only one paragraph generate karnage
    if(isNaN(value) || value < 0 || value > 9){
        // code by mahi start
        // getText = Math.floor(Math.random() * text.length) ;
        // getTextContent = text[getText];
        // result.innerHTML = getTextContent;
        // code by mahi end
